...
class UGATIT(object) :
    ...
    def build_model(self):
        ...
        aug_para = {}
        if self.aug_sigma:
            aug_para["sigma"] = self.aug_sigma
            aug_para["points"] = self.aug_points
            aug_para["rotate"] = self.aug_rotate
            aug_para["zoom"] = self.aug_zoom

        self.data_iter = common_pelvic.DataIter(self.device, self.data_dir, patch_depth=self.img_ch,
                                                batch_size=self.batch_size, aug_para=aug_para, mini=self.mini)
        if self.do_validation:
            self.val_data_s, self.val_data_t, _, _ = common_pelvic.load_val_data(self.data_dir, mini=self.mini)
